...
Full-stack Todo application monorepo using pnpm 11 workspaces. Backend is NestJS with Prisma ORM (PostgreSQL), frontend is Next.js 16 with React 19. Node 24 required (see `.node-version`).
...
Workspace layout is defined in `pnpm-workspace.yaml` (not the `workspaces` field of `package.json`).
That file also holds all pnpm settings — since pnpm 11, `.npmrc` is only read for auth/registry
settings and the `pnpm` field of `package.json` is ignored.

### Backend

```sh
pnpm --filter backend start:dev                  # Dev server with watch (port 4000)
...
pnpm --filter backend exec prisma generate       # Generate Prisma client (required before build)
pnpm --filter backend exec prisma migrate dev    # Create/apply migrations
```
...
pnpm --filter frontend build   # Static export build
pnpm --filter frontend lint    # Next.js lint
pnpm --filter frontend orval   # Generate API types from backend OpenAPI spec
```

### Linting & Formatting (Biome)

```sh
pnpm fix:all        # Lint + format both packages
pnpm fix:frontend   # Frontend only
pnpm fix:backend    # Backend only
```
...
When backend API changes: restart backend to regenerate `openapi.yml`, then run `pnpm --filter frontend orval` to update generated types.

### Auto-Generated Files (Do Not Edit)

- `packages/backend/generated/prisma/` — Prisma client (regenerate with `pnpm --filter backend exec prisma generate`)
- `packages/frontend/app/gen/` — Orval API types and hooks (regenerate with `pnpm --filter frontend orval`)
